...
L'assembler traduce un file in linguaggio assebmly in un file binari in linguaggio macchina
...
in altri file. Un'etichetta (label) è esterna (global) se l'oggetto etichettato se si può fare riferimento
ad essa anche in altri file oltre a quello in cui è definita. Viceversa è locale se ci si può riferire
...
sezioni distinte
\begin{enumerate}
    \item l'object file header, che descrive la dimensione e la posizoone delle altre parti del file
...
\end{enumerate}
La produzione di diversi file oggetti è causata dalla compilazione separata, che permette di
...
subroutine predefiniti e strutture dati a cui il programma fa riferimento.
...
lo spazio di memoria che occuperà ogni modulo, dato che l'assebler compilando i moduli
