...
\risposta{
    Una struttura dati per insiemi disgiunti serve a mantenere una collezione di insiemi disgiunti.
    Le operazioni principali sono: Make-set(x), che che crea un nuovo insieme con x come unico elemento, Find-set(x) che restituisce un puntatore al rappresentante del set in cui è contenuto x,
    e Union(x,y) che unisce gli insiemi di cui x e y fanno parte.
}
...
\domanda{21}{
	Definire formalmente cos'è un albero di compertura minimo(MST)?
	Illustrare l'approccio greedy generico (fornire la procedura GENERIC-MST) per determinare un MST illustrando il principio di funzionamento (cosa fa la proceduara ad ogni iterazione, proprietà invariante di ciclo, nozione di arco sicuro, ecc).
	Come si trova un'arco sicuro?
	definizioni di: taglio, archo che attraversa un taglio, taglio che rispetta un sottoinsieme di archi, arco leggero.
	Teorema che dà una condizione sufficiente affinchè un arco sia sicuro.
}
...
\domanda{23}{
	Complessità computazionale dell'algoritmo di Floyd-Warshall (giustificare)
}
\end{document}