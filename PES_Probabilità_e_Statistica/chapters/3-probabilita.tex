...
Elenchiamo ora alcune probabilità importanti della Probabilità Condizionata:

\formula{Regola del prodotto}{$P(A \cap B) = P(B)\cdot P(B|A)$}
La probabiltà dell'intersezione di $A$ e $B$ è data dal prodotto tra $P(B)$ e $P(A|B)$, infatti:
$P(B) \cdot \frac{P(A \cap B)}{P(B)} = P(A\cap B)$
\formula{Formula di Disintegrazione}{$ P(A) = P(A \cap B) + P(A \cap B^c)$}
\formula{Formula delle probabilità totali}{$P(A) = P(A|B)P(B) + P(A|B^c)P(B^c)$}
