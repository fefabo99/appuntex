...
% \section{Notazione} Con $X_i$ indichiamo la Variabile Aleatoria $X_i: \Omega \rightarrow \R$
...
\definizione{
	Una Variabile Aleatoria $Z$ si dice \textbf{Normale Standard} e si scrive:
	\[Z \sim N(0,1)\]
	...
	\]
	\begin{center}
		\includegraphics[width=.4\textwidth]{va normale.png}
	\end{center}
	In questo caso i valori assunti sono: $ Z(\Omega) = (-\infty, + \infty)$
}
...
\subsection{Variabili Normali Centrate}
Siano ora $\mu \in \R$, $\sigma \in (0, \infty)$.
\definizione{
	Una Variabile Aleatoria $X$ si dice \textbf{Normale con media $\mu$ e varianza $\sigma^2$} e si scrive
	\[
		X \sim N(\mu, \sigma^2)
	\], se X è assolutamente continua e:
	\begin{center}
...
\teorema{
	Se $X$ è normale $\rightarrow$ $Y = aX+b$ è Normale
	\\$\forall a \neq 0, b \in \R$
...
\teorema{ $X$ e $Y$ normali \textbf{indipendenti} $\rightarrow X+Y$ è normale.
	\[
...
\esempio{
	$X\sim N(0,1)$ e $Y\sim N(0,1)$ Indipendenti. Allora:
