...
\paragraph{Altre proprietà del determinante}
...
Il teorema di Cramer definisce una relazione tra il Determinante e i Sistemi di Equazioni Lineari.
\teorema[di Cramer]{
...
\subsection{Criterio dei Minori} %Relazioni Determinante-Rango
...
\definizione{
	Una \textbf{sottomatrice} di $A$ è una matrice ottenuta rimuovendo delle righe e/o colonne di $A$.
	\\Un \textbf{Minore} di $A$ è il \emph{determinante di una sottomatrice quadrata di $A$}. L'ordine di un minore è l'ordine della sua sottomatrice.
}
Da qui, possiamo enunciare il teorema che definisce la relazione tra determinante e rango:
\teorema{
	Sia $A$ una matrice non necessariamente quadrata, allora il \underline{\textbf{Rango}} di $A$ è uguale al \textbf{massimo ordine dei minori} non nulli di $A$.
}
\esempio{
	...
	\begin{enumerate}
		...
		\item Dal teorema precedente se esibisco una sottomatrice di ordine 3 con $det\neq 0$ ho dimostrato che $rg(A) = 3$.
	...
	Questo significa che dovrei calcolare il determinante di ogni sottomatrice $3\times 3$ finchè non ne trovo uno non nullo, il che richiederebbe molti calcoli.

	D'altro canto posso usare le seguenti \textbf{considerazioni per risparmiarmi i calcoli}:
	\begin{itemize}
		\item $det(B)\neq 0 \Longleftrightarrow $ Colonne di $B$ linearmente indipendenti $\Longleftrightarrow  rg(B)$ massimo.
		\item Se due colonne sono indipendenti, allora anche "allungandole" rimangono linearmente indipendenti.
	\end{itemize}
	Da queste due considerazioni posso prendere la matrice $2\times 2$ che ho preso in precedenza, ed essendo le due colonne linearmente indipendnenti lo sono anche le colonne complete di lunghezza 3.
	\\Da qui mi basta prendere una \textbf{terza colonna linearmente indipendente} ad esse (per esempio $c_4$) per avere una sottomatrice $3x3$ con le colonne linearmente indipendenti, quindi determinante diverso da $0$.
	\end{enumerate}

	Quindi, prendendo la sottomatrice $3\times 3$ fatta dalle colonne $c_1,c_2,c_4$ noto che ha determinante diverso da zero, quindi $rg(A)=3$.
}

\section{Matrici Inverse}
\definizione{
	Sia $A$ una matrice quadrata. Essa è \textbf{Invertibile} \emph{se e solo se}:
	\[ \exists B: A\cdot B = B\cdot A = Id \]
	In tal caso $B$ è detta \textbf{Matrice Inversa} di $A$ e si indica con:
	\[ B = A^{-1}\]
}
Come facciamo a sapere se una matrice è invertibile?
\teorema{
	Una matrice è invertibile \emph{se e solo se} il suo \textbf{determinante è non nullo}.

	In tal caso, l'elemento della matrice inversa $A^{-1}(x_{ij})$ è dato da:
	\[
		x_{ij}= (-1)^{i+j} \cdot \frac{det(A_{ji})}{edt(A)}
	\]
	In cui $A_{ji}$ è la sottomatrice ottenuta rimuovendo la $j$-esima riga e la $i$-esima colonna da $A$.
}
...
\subsection{L'Algorimto di Gauss-Jordan}
