...
\definizione{
    Definiamo \textbf{Combinazione Lineare} di $\{\underline{v}_i\}_{i=1,...,n}$ una qualsiasi espressione del tipo
    \[\sum_{i=1}^{n} \lambda_i \cdot \underline{v}_i\]
...
\osservazione{In parole povere un insieme di $n$ vettori sono \textbf{linearmente indipendenti} se e solo se
per ottenere 0 dalla loro combinazione lineare è $\underline{0}$ devo porre tutti i coefficienti a 0:
\[ \alpha A + \beta B + \gamma C = 0 \implies \alpha = \beta = \gamma = 0\]
}
...
\definizione{
    La dimensione di uno spazio vettoriale $V$ che ammette un sistema di generatori finito
     è il \textbf{numero di elementi di una base qualsiasi} di $V$.
}
...
%Lezione 6
\subsection{Teorema di De Guzman}
Sia $V$ spazio vetoriale e $W,Z<V$, tutti a dimensione finita.
\\Allora avremo che:
\begin{itemize}
    \item $W\cap Z < V$
    \item $W + Z = \{w+z : w\in W, z\in Z \}$
\end{itemize}
\osservazione{Posso sommare $W$ e $Z$ perchè sono entrambi sottospazi di $V$, ed il risultato
è il sottospazio di $V$ che contiene $W\cup Z$.
}
\[dim(W+Z) = dim(W) + dim(Z) - dim(W\cap Z)\]


\subsection{Esercizi: Trovare una Base}
...
Uno di essi prevede il passaggio dalla forma cartesiana alla forma parametrica, vediamolo con un esempio:
 \esempio{
    Abbiamo 
    \[V = \{ (x,y,z)\in \R^3 | 2x + 3y +z = 0\}\]
    $V$ è un sottospazio vettoriale di $\R^3$ ed è rappresentato in forma cartesiana.
    Per passare alla forma parametrica dobbiamo appunto introdurre dei parametri,
    Ce ne serviranno $n-m$, dove $n$ è la dimensione dello spazio e $m$ il numero di equazioni.
    In questo caso abbiamo che $n=3$ e $m=1$, quindi ci servono 2 parametri, che chiameremo $t$ e $s$.
    \\Impostiamo quindi:
    \[
     \begin{cases}
         x=t \\ y=s z=-2t-3s
     \end{cases}    
     \to V = \{ (t,s,-3t-3s) | t,s \in \R\}
    \]
    Avendo ora la forma parametrica, posso trovare una base di $V$, scegliendo $t=1,s=0$ e $t=0,s=1$.
    \[
    t=1,s=0 \implies v_1 = (1,0,-2)  
    \]
    \[
    t=0,s=1 \implies v_2 = (0,1,-3)  
    \]
    $\{v_1,v_2\}$ è una base di $V$.
 }
