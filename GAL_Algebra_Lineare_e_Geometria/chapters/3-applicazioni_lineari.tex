...
\subsubsection{Dimensione del Nucleo}
...
\definizione{Un'applicazione lineare $f$ è iniettiva se e solo se la dimensione del suo nucleo é $0$:
\[ dim(Ker(f)) = 0 \implies \;\; f\text{ è iniettiva} \]
}
...
\definizione{
	...
	\[ dim(Im(f)) = dim(W) \implies \;\; f\text{ è suriettiva} \]
}
Si può anche affermare che:
\begin{itemize}
