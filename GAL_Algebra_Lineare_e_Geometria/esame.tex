...
\domanda{49}{Siano $A$ e $B$ due matrici $n\times m$ diverse. E' possibile che esista una matrice quadrata $C$ tale che $C\cdot A = C\cdot B$?}
\rispostechiuse{Se la dimensione dello spazio vettoriale generato dai vettori riga di $A$ è minore di $n$}{Sse $det(C)\neq 0$}{sse $det(C)=0$}{no}
\risposta{\textbf{e}
% \\ Noi abbiamo che $CA = (CA_1,...,CA_m)$ e $CB = (CB_1,...,CB_m)$ (in cui $CA_i$ è la iesima colonna di $CA$.)
% \\ Siccome $CA = CB$ allora vuol dire che $CA_i = CB_i \forall i$,
% \\ ma abbiamo che $A\neq B$ quindi $\implies \exists j t.c. A_j \neq B_j$
Non ho capito la spiegazione del prof
}

\subsection{Omomorfismi}
\domanda{25}{Sia $f_t:\R^3\to\R^3$ un omomorfismo che manda il piano di equazioni $2x+y-z=2$ nel piano di equazioni $y-tx-2z=0$, per $t\in \R$. è possibile che $f_t$ sia iniettiva?}
\rispostechiuse{è sempre iniettiva}{Può essere iniettiva solo per un numero finito di $t$}{Può essere iniettiva, tranne per unn numero finito di $t$}{Non è mai iniettiva}
\rispostaaperta{\textbf{d}
\\Un omomorfismo si definisce iniettivo se $dim(ker(f))=0$. 
\\In questo caso avremo che $dim(im(f)) = 2$ e $dim(dom(f))=3$, quindi per il teorema nullità più rango:
\[ dim(dom(f)) = dim(Ker(f)) + dim(Im(f)) \implies dim(ker(f))=1\]
Di conseguenza non può essere iniettiva.
\osservazione{La dimensione di un piano in cui passa l'origine è 2, mentre la dimensione di un piano in cui non passa l'origine è 3.}
}
...
\domanda[Omomorfismi]{Quali dei seguenti omomorfismi $\R^2\to \R^2$ manda il quadrato di vertici $(0,0),(0,3),(3,0),(3,3)$ nel disco chiuso centrato in (0,0) e di raggio 2?}
%\rispostachiusa{\textbf{d}$(1,0)\to(1/2,1/3) e (0,1)\to(-1/4,1/4)$}
