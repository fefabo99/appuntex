...
\part{Esami Primo Semestre}
\section{Febbraio 2021}
\subsection{Domande Chiuse}
\domanda[Equivalenza asintotica]{1}{Sia $a_n = \frac{n \ln (1- \frac{2}{n^3})}{n \sqrt[3]{n} - n^3}$. Allora, per $n \to +\infty$,
}
...
\domanda[Massimo/minimo]{2}{Sia $f:\R \to \R$ con $f'(0)=0,f''(x)=ln(e+x)$.Allora $f$ ha in $x=0$}
...
\domanda[Proprietà degli integrali]{3}{Sia $f:\R \to \R$ una funzione continua e dispari. Allora, $\int_{-3}^{4} f(x) dx$ è uguale a}
...
\domanda[derivata]{4}{La derivata della funzione $f(x) = \sqrt[3]{\frac{x^3}{2} +1}$ è}
...
\domanda[convergenza di una serie]{5}{la serie $\serie{1}{+\infty} \frac{1}{n^{(\alpha+1)/2}\ln^2n}$ }
...
\domanda[continuità di una funzione]{6}{La funzione $f(x) = \begin{cases}a \sin x - b^2  & -2 \leq x \leq 0 \\ 1-e^x &0<x\leq 3\end{cases}$ è derivabile in $x=0$ se e solo se:}
\risposta{$a=-1,b=0$}
\spiegazione{
...
\domanda[intervalli]{7}{Quali tra questi insiemi è un intervallo?}
\risposta{$\{x \in \R: 2|x| \geq x^2\}$}
\spiegazione{La domanda chiede quali delle disequazioni proposte genera \emph{un solo intervallo}.
\\Risolvendo ogni disequazione (usando il metodo per risolvere le disequazioni con il modulo) si trova che solo una genera un intervallo unico, infatti:
$$\{x \in \R: 2|x| \geq x^2\} = [-2,0]\cup[0,2] = [-2,2]$$
}


\domanda[composizione di funzioni]{8}{Date le funzioni $f(x) = \ln(x), g(x)= x^3, h(x) = 2-x$, la funzione composta $(h \circ g \circ f)(x)$ è:}
\risposta{$2-\ln(x)$}
...
\spiegazione{
	...
	$$
	la corrispondenza asintotica vale perchè \emph{l'argomento del seno è infinitesimo}.
	La successione risultante è una serie armonica di grado $> 1$, quindi converge.
}
\domanda[CONFRONTO TRA INFINITI]{2}{La Successione $a_n = \frac{\ln(2+n^3)-5\sqrt[]{n^2-n}+2^{-n^4+5n}}{5n+3\ln n - n \ln n}$ per $n \to +\infty$ ha limite:}
\risposta{$\limite{n}{+\infty} = 0$}
\spiegazione{
	Il denominatore è inequivocabile che sia $\sim 5n$, essendo \emph{l'infinito di ordine maggiore}.
	\\Al numeratore invece sembra più complicata:
	\begin{itemize}
		\item[a] $2^{-n^4+5n} \to 0$
		\item[b] $-5\sqrt{n^2-n}\to -\infty$
		\item[c] $\ln(2+n^3) \to +\infty$
	\end{itemize}
	Tra i tre termini possiamo ignorare "a", mentre tra "b" e "c" l'infinito più rapido tra \emph{logaritmo e radice è la radice}.
	\\Quindi abbiamo $\sim \frac{-5\sqrt{n^2-n}}{5n}$, ed essndo $\alpha \gg \sqrt{\beta}$, la successione \emph{tende a 0}.
	%%VECCHIA SPIEGAZIONE, probabilmente sbagliata
...
\domanda[ASINTOTO OBLIQUO]{3}{La funzione $f(x) = \frac{2x^3+4x}{2-x^2} + e^{-\frac{1}{x}}$, per $x\to +\infty$, ha asintoto obliquo di equazione:}
\risposta{$$y=-2x+1$$}
\spiegazione{
...
\domanda[DERIVATA]{4}{Sia $f(x) = \sqrt{x^2 +2x +3}$. allora $f'(1)$ vale:}
...
\domanda[DISEQUAZIONE]{5}{L'insieme delle soluzioni della disequazione $e^x \sqrt[3]{x-1} \geq 1$ è del tipo}
...
\domanda[MONOTONIA]{6}{la funzione $f(x) = \ln(x^2+2x+3)$ è monotona crescente se e solo se}
...
\domanda[ESTREMO SUCCESSIONE]{7}{L'estremo inferiore della successione $\{a_n\}_{n\geq0}$, dove $a_n = 3^{n+(-1)^nn}$ è:}
\risposta{1}
\spiegazione{
...
\domanda[INTEGRALE]{8}{$\int_{\pi/2}^{\pi} x \sin x dx$ vale:}
\risposta{
	$\pi -1$
}
\spiegazione{
...
\\\textbf{iii.} si calcoli la somma della serie $\serie{1}{+\infty}(\ln(n+1)-\ln n)$
	...
	\section{Giugno 2020}
	\subsection{Domande Chiuse}
	\domanda[SURIETTIVITÀ/INIETTIVITÀ]{1}{La funzione $f:\R \to \R$ definita da $f(x)=\begin{cases}e^x & x>0 \\ x+2 & x\leq 0\end{cases}$ è}
	...
	\spiegazione{
		...
		Per verificarlo basti vedere che nel grafico della funzione dei due segmenti si accavallano su alcuni valori di $y$.
	}

	\domanda[COMPOSIZIONE]{2}{Siano $f(x) = e^{\sqrt{x}}$ e $g(x)= \ln(1+x)$. Allora $(g \circ f)$}
	...
	\domanda[SERIE GEOMETRICA]{3}{La somma della serie $\serie{1}{+\infty} e^{1-2n}$ vale}
	...
	\domanda[CONFRONTO TRA INFINITI]{5}{$\limite{n}{+\infty} \frac{n^2 \ln^3 n - n^3 \ln^2 n +4 \arctan n}{n^3 \ln^2 n - n^2 \ln^5 n- e^{-n^2}}$ vale:}
	\risposta{-1}
	\spiegazione{$\frac{n^2 \ln^3 n - n^3 \ln^2 n +4 \arctan n}{n^3 \ln^2 n - n^2 \ln^5 n- e^{-n^2}} \sim \frac{-n^3 \ln^2 n}{n^3 \ln^2 n} = -1$}

	\domanda[MASSIMO DI UN INSIEME]{6}{Il massimo dell'insime $A = {\frac{2+(-1)^n}{2^n+(-1)^{n+1}}}$ è}
	\risposta{1}
	\spiegazione{per $n=2$ l'insieme vale 1, mentre per tutti gli altri valori di n è minore.}


	\domanda[DERIVATA]{7}{sia $f(x) = \arctan(\sqrt{x})$. Allora $f'(1)=$}
...
\section{Giugno 2021}
\subsection{Domande Chiuse}
\domanda[CONCAVITÀ/CONVESSITÀ]{O1}{Sia $f:\R \to \R$ definita da $f(x) = x^3 + 4x$. sull'intervallo $I = (-1,1)$ la funzione $f$ è:}
...
\domanda[SERIE GEOMETRICA]{O2}{La somma della serie $\serie{0}{+\infty} (\frac{2}{3})^{2n}$ vale}
\risposta{$\frac{9}{5}$}
...
\domanda[composizione di funzioni]{O3}{Siano $f(x) = \ln(x^2+1)-1$ e $g(x) = |x+1|$. Allora $(g \circ f)(x) = $}
\risposta{$ln(x^2 + 1)$}
\spiegazione{
...
\domanda[Estremo di un insieme]{O4}{L'estremo superiore dell'insieme $A={\frac{1}{n+1} +2^{1-2n}, n= 1,2,...}$ è}
\risposta{1}

\domanda[iniettività/suriettività]{O5}{La funzione $f:\R \to \R$ definita da $f(x) = \begin{cases} e^{-x} & x>0 \\ x^2-1 &x\leq 0 \end{cases}$ è}
...
\domanda[integrale]{O6}{L'integrale definito $\int_{0}^{2} \frac{x}{x^2+1} dx$ Vale:}
...
\domanda[CONFRONTO TRA INFINITI]{O7}{$\limite{n}{+\infty}(\ln n - \sqrt{n}+3e^{-n} +\sin n^2)$ Vale:}
\risposta{$-\infty$}
...
\domanda[Derivata di funzione composta]{O8}{Siano $f,g : \R \to \R$ derivabili. se $f(2) = -2, f'(2)=4,g'(-2) = -4}$, allora $(g\circ f)'(2)$ Vale:
\risposta{-16}
\spiegazione{Avendo queste funzioni valutate, si può cercare di "indovinare" la loro espressione.
\\Quindi, partendo dalle derivate si cerca il l'espressione più semplice che verifica la sua valutazione, si "antideriva" e si somma/sottrae una costante per avere il risultato desideato:

\begin{itemize}
	\item $f'(2)=4 \implies f'(x)=2x$
	\item $f(2)=-2 \wedge f'(x)=2x \implies f(x) = x^2 -6$
	\item $g'(-2) = -4 \implies g'(x)=2x \implies g(x)=x^2$
\end{itemize}
A questo punto si compone, si deriva (con la formula della derivata di una funzione composta) e si valuta la derivata per ottenere il risultato.
}
...
\subsection{Domande Aperte}
	...
	\section{Giugno 2022}
	\subsection{Domande Chiuse}
	\domanda[convergenza assoluta]{1}{La serie $\serie{1}{+\infty} (-1)^n \frac{1}{n\sqrt{n}}$}
	\risposta{Converge assolutamente}
	...
	\section{Luglio 2019}
	\subsection{Domande Chiuse}
	\domanda[Convergenza di una serie]{1}{La serie $\serie{2}{+\infty} \frac{n^\alpha}{n^3\ln n}$ converge sse:}
	\risposta{$\alpha<2$}
	\spiegazione{
	...
	\domanda[confronto tra infiniti]{2}{$\limite{n}{+\infty} \frac{n^2-3\ln^{10}n-n\sqrt{n^3+1}}{2n^2+e^{\frac{1}{n}}-n\sqrt{n}}$ è}
	\risposta{$-\infty$}
	...
	\\Al numeratore prevale quindi il $n\sqrt{n^3+1}$
	\\Al denominatore è bene notare che $e^\frac{1}{n}$ tened a 1 (radice $n$-esima).
	...
	\domanda[derivata]{4}{Sia $f(x) = xe^{\sqrt{x}}$. Allora $f'(4)$ vale:}
	\risposta{$2e^2$}
	...
	\domanda[disequazione]{5}{L'insieme delle soluzioni della disequazione $\sqrt{2x^2-1}>-1$ è}
	\risposta{$(-\infty,-\frac{1}{\sqrt{2}}]\cup[\frac{1}{\sqrt{2}},+\infty)$}
	\spiegazione{Una \emph{radice di indice pari è sempre positiva}, quindi è anche sempre maggiore di $-1$.\\
		Bisogna però vedere in valori in cui l'argomento della radice è maggiore o uguale a 0 (nei valori in cui l'argomento è \emph{minore di 0} la radice, e quindi la disequazione, \emph{non esiste}).
		\\Quindi $2x^2-1\geq 0$, quindi ottengo l'intervallo della risposta.}
	
	\domanda[Punti di flesso]{6}{Tutti e soli i punti di flesso di $f:\R \to \R$, $f(x)=x^4-3x^2+14$ sono:}
	...
	\domanda[Condizione di Integrabilità]{7}{Sia $f:[a,b] \to \R$. Per l'integrabilità di $f$ su $[a,b]$ la condizione che $f$ sia continua è:}
	...
	\domanda[integrale]{8}{$\int_{1}^{e^2} \frac{\ln^2x}{x}dx$ vale}
	\risposta{$\frac{8}{3}$}
	\spiegazione{
	...
	\section{Luglio 2020}
	\subsection{Domande Chiuse}
	\domanda[crescenza/decrescenza]{1}{Sia $f:\R \to \R$ definita da $f(x) = x^3 - 3x$, sull'intervallo I = (-1,1) $f$ è}
	...
	\domanda[dominio]{2}{Il dominio della funzione $f(x)= ln(4-x^2)$ è}
	\risposta{(-2,2)}
	\spiegazione{Gli argomenti dei logaritmi devono essere $>0$}
	\domanda[Serie geometrica]{3}{La somma della serie $\serie{1}{+\infty} 4^{-n}$ vale}
	\risposta{$\frac{1}{3}$}
	...
	\domanda[confronto tra infiniti]{5}{
		$\limite{n}{+\infty}(n^2-n^3+3e^{-n} + \cos n^2)$ vale
	}
	\risposta{$-\infty$}
	\spiegazione{$a_n \sim -n^3$}
	\domanda[limite di un insieme]{6}{SUP $A=\{\frac{2}{2n^2+1} + e^{1-n},n=1,2,...\}$ è}
	\risposta{$\frac{2}{3}$}

	\domanda[Derivata dell'inversa]{7}{Detta $g$ funzione inversa di $f(x) = x^3+x+1$, allora $g'(3)=$}
	...
	\domanda[Continuità di una funzione]{8}{La funzione $f(x)=\begin{cases} \sin x^2 + a & x\leq 0 \\ \frac{\ln(1+x)}{x+x^3} & x>0 \end{cases}$ è continua se:}
	\risposta{$a=1$}
	\spiegazione{
	...
	\rispostaaperta{
		Per controllare se una delle due successioni è $o$-piccolo dell'altra bisogna vedere se un limite di uno dei rapporti tende a 0.
		\\Si può andare a tentativi, ma guardando subito il rapporto di $\frac{b_n}{a_n}$ si nota che:
		$$\frac{b_n}{a_n} = \frac{\frac{n\sqrt{n}-\ln n}{n+e^n}}{\frac{n^2+\ln n}{n\sqrt{n}+e^{-n}}} = \frac{n\sqrt{n}-\ln n}{n+e^n} \cdot \frac{n\sqrt{n}+e^{-n}}{n^2+\ln n}$$.
		Da qui si può vedere che nel limite di $n\to +\infty$ il denominatore di $b_n$ tende fortemente a $+\infty$ (grazie a $e^n$).
		Questo porta l'intera successione a tendere a $0$, quindi $b_n = o(a_n)$
	}
...
\domanda[Continuità di una funzione]{O1}{La funzione $f(x)=\begin{cases} \sin x^2 + a & x\leq 0 \\ \frac{\ln(1+x)}{2x} + \frac{3}{2} &x>0\end{cases}$ è continua se:}
\risposta{$a=2$}
...
\domanda[Derivata]{O2}{Sia $f(x) = x^2+2x+2$. allora $\frac{d}{dx} \ln(f(x))$ per $x=1$ è }
\risposta{$\frac{4}{5}$}
\spiegazione{la derivata di $\ln(f(x)) = \frac{2x+2}{x^2+2x+2}$, che per $x=1$ vale $\frac{4}{5}$}

\domanda[Punti di flesso]{O3}{La funzione $f(x) = x^5+x^3-1$ ha quanti flessi?}
\risposta{1 flesso}
\spiegazione{Calcolo la derivata seconda, che è banale $f''(x)=20x^3+6x$ e la pongo maggiore di 0:
\\$f''(x)>0 = 20x^3+6x>0 \to x(20x^2+6)$
\begin{itemize}
	\item $x>0$
	\item $20x^2+6>0$ non ha soluzioni perchè ha $\Delta<0$
\end{itemize}
Di conseguenza $f''(x)>0$ per $x>0$, quindi ha 1 flesso.
}

\domanda[integrale]{O4}{$\int_{0}^{1} xe^x dx =$}
\risposta{1}
\spiegazione{
		Integro per parti:
...
\domanda[Convergenza di una serie]{O7}{La serie $\serie{1}{+\infty} \frac{n^2}{n\ln n + 2n^{\alpha+1}}$}
...
\rispostaaperta{
	\paragraph*{1} Integrando per parti (banale) trovo $F(x) = \sin x - x\cos x + C$
	\paragraph*{2} DA FARE
...
\domanda[Convergenza assoluta]{1}{La serie $\serie{1}{+\infty}(-1)^n \frac{1}{n^3}$}
\rispostechiuse{\rispostagiusta converge asolutamente}{converge, ma non assolutamente}{diverge}{è irregolare}

\domanda[confronto tra infiniti]{2}{$\lim_{n\to +\infty} \frac{n^3+5ln^2n-n^2\sqrt{n^3+1}}{-n^3+e^{1/n}-n^2\sqrt{n}}$ è}
\rispostechiuse{$-\infty$}{\rispostagiusta $+\infty$}{1}{0}

\domanda[Concavità/convessità]{3}{La funzione $f(x) = x^2 + 2\ln x$ è convessa se e solo se}
\rispostechiuse{$x\in (-1,1)$}{$x\in(0,1)$}{\rispostagiusta $x\in(1,+\infty)$}{$x\in(0,+\infty)$}
\spiegazione{Attenzione al Dominio della funzione!}

\domanda{4}{La funzione $f(x) = \begin{cases}\frac{ln(1+x^2)}{x} & x>0 \\ 1+ k\cos x & x\leq 0 \end{cases}$ è continua in $x=0$ se e solo se}
\rispostechiuse{k=0}{k=1}{\rispostagiusta k=-1}{per nessun valore di k}

\domanda{5}{L'insieme delle soluzioni della disequazione $\sqrt{4-x^2}\leq \sqrt{3}$ è}
\rispostechiuse{\rispostagiusta $[-2,-1]\cup [1,2]$}{$(-\infty,-1]\cup[1,+\infty]$}{$[-1,1]$}{$(-2,-1]\cup[1,2)$}

\domanda{6}{la funzione $f(x) = xe^x -3e^x$ ha}
\rispostechiuse{un punto di massimo globale}{\rispostagiusta un punto di minimo globale}{un punto di minimo locale ma non globale}{un punto di massimo locale ma non globale}

\domanda{7}{Sia $a_n = \frac{1}{n^2+n}$ e $b_n = \frac{1}{n}$. Allora}
\rispostechiuse{$a_n \sim b_n$}{\rispostagiusta $a_n=o(b_n)$}{$b_n=o(a_n)$}{nessuna delle alternative proposte}
\spiegazione{Mettere spiegazione}

\domanda{8}{L'integrale $\int_{-2}^{5} \sqrt[3]{x+3}dx$ vale}
\rispostechiuse{3}{315/4}{\rispostagiusta 45/4}{7/8}
...
\rispostaaperta{\textbf{1.1}
\paragraph*{Dominio} Denominatori $\neq 0$ e argomento dei logaritmi $> 0$, quindi:
\begin{itemize}
	\item $4x^2\neq 0 \to x\neq 0$
	\item $x>0$
\end{itemize} 
...
\begin{itemize}
	\item $\limite{x}{+\infty} f(x) = \frac{\infty \ll}{\infty \gg} = 0$
	\item $\limite{x}{0^+} f(x)= \frac{-\infty}{0} = -\infty$
\end{itemize}

\paragraph*{Asintoti}
\begin{itemize}
	\item $y=0^+$ è Asintoto Orizzontale per $x\to +\infty$
	\item $x=0 $ è Asintoto Verticale
\end{itemize}
\paragraph*{Derivata}
$$f'(x) = \frac{1}{4} \cdot \frac{\frac{1}{x} \cdot x^2 - 2xln(x)}{x^4} = ... = \frac{1-2\ln(x)}{4x^3}$$

\paragraph*{Monotonia} $f'(x)>0$
Il denominatore è \emph{Sempre maggiore di 0}, mentre il numeratore:
$$N>0 \to ... \to \ln(x)<\frac{1}{2} \to x<e^{\frac{1}{2}} \to x<\sqrt{e}$$
Quindi $f'(x)>$ solo per $x<\sqrt{e}$. $\sqrt{e}$ è anche punto di MASSIMO GLOBALE.

\paragraph*{Derivata Seconda}$f''(x) = $ ometto il calcolo (da mettere)
$$f''(x)= \frac{6\ln(x)-5}{x^4}$$

\paragraph*{Concavità/Convessità}
$f''(x)>0$\\
\begin{itemize}
	\item $N>0 \to 6ln(x)>5 \to ln(x)>\frac{5}{6} \to x>e^{\frac{5}{6}}$
	\item Il denominatore è sempre positivo
\end{itemize}
Quindi $f''(x)>0$ per $x>\sqrt[6]{e^5}$. è concava tra 0 e $\sqrt[6]{e^5}$, convessa fino a $+\infty$.
\\$\sqrt[6]{e^5}$ è anche Punto di Flesso.
\paragraph*{grafico}
La funzione segue l'asse delle y a $0^+$, sale fino a sopra $\sqrt{e}$ poi scende e si appogga sopra all'asse delle x. 
}
\rispostaaperta{\textbf{1.2}\\
Per calcolare la retta tangente ad $f$ in $x=e$, calcolo $m = f'(e)$, e poi $q=f(e)-f'(e)\cdot e$.
\begin{itemize}
	\item $f(e)= \frac{1}{4e^2}$
	\item $f'(e)= -\frac{1}{4e^3}$
\end{itemize}

\begin{itemize}
	\item $m= -\frac{1}{4e^3}$
	\item $q=\frac{1}{4e^2} - \frac{1}{4e^3} \cdot e = \frac{1}{2e^2}$
\end{itemize}

Quindi l'equazione della retta è $y=\frac{x}{4e^3} + \frac{1}{2e^2}$.
}

\rispostaaperta{\textbf{1.3}\\
Trovo una primitiva di $f(x)$ integrando per parti:
$\int \frac{\ln x}{4x^2} = \frac{1}{4}\int \frac{\ln x}{x^2}$. 
\\Ho $f = \ln x \to f'=\frac{1}{x}$ e $g'=x^{-1} \to g = - \frac{x-1}{-1} = -x^{-1}$.
\\Applicando la formula ho $\frac{1}{4} \cdot -\frac{\ln x}{x} - \int \frac{1}{x} \cdot - \frac{1}{x} = -\frac{\ln x}{4x}$.
Calcolando $F(4) - F(1) = \frac{-ln(4)-3}{16}$. 
}
...
\rispostaaperta{\textbf{2.1}\\
	Perchè la serie, che è di tipo geometrica, converga abbiamo bisogno che il suo "argomento" q sia compreso tra $-1$ e $1$.
	\\Quindi $|\frac{1}{x-1}| < 1 \implies x>2 \wedge x<0$.
}
\rispostaaperta{\textbf{2.2}\\
La serie parte da $n=2$, quindi alla formula risolutiva bisogna togliere $q^1$ e $q^0$.
$$\serie{2}{+\infty}(\frac{1}{x-1})^n = \frac{1}{1-q} - q - q^0 = \frac{1}{(x-2)(x-1)}$$
}
\end{document}