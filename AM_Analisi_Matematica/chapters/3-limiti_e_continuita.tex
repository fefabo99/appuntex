...
Il teorema di permanenza del segno assicura l'esistenza di un intorno di un punto $c$ in cui 
una funzione $f(x)$ é concorde con il limite di $f(x)$ per $x \to c$, a patto che tale limite sia diverso da $0$.

\definizione{
	Sia $f(x):\R \to \R$ e sia $c$ un punto di accumulazione per il dominio.
	
	$$
		\text{Se } \lim_{x\to c} f(x) = l > 0 \implies f(x) > 0 \forall x\in I_c
	$$
	$$
		\text{Se } \lim_{x\to c} f(x) = l < 0 \implies f(x) < 0 \forall x\in I_c
	$$
	con $I_c$ intorno di $c$
}
