...
Per ogni $x\in \R$, consideriamo la serie di termine generale $x^k$, cioè la serie
\begin{equation*}
    \sum_{n=0}^{+\infty} x^n
\end{equation*}
Essa è detta \emph{serie geometrica di ragione $x$}.
è uno dei (pochi) casi in cui si riescono a calcolare esplicitamente, al variare di $x$, le somme parziali $s_n$.
...
risulta:
\begin{equation}
    \underset{x \to \infty}{\lim} s_n \begin{cases}
        \text{non esiste} & x \leq -1 \\
        = \frac{1}{1-x} & -1 < x < 1 \\
        = +\infty & x \geq 1
    \end{cases}
\end{equation}
