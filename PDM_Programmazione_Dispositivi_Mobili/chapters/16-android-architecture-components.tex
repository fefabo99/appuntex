...
\par La volta scorsa abbiamo visto architettura generica, S.O.L.I.D. e Clean Architecture e Dependency Injection, ovvero il fatto di appoggiarsi su astrazioni (interfacce) piuttosto che entità concrete per favorire il riutilizzo.
...
\par A seguito di un cambiamento di \textbf{stato} dei componenti Android occorre effettuare delle \textbf{azioni di risposta} (es. mettere in pausa la camera a seguito di uno stato \textit{PAUSED}). Di norma sono codificate nei metodi del ciclo di vita del componente (es.: nelle \texttt{Activity}, nei metodi \texttt{onStop()}, \texttt{onStart()}\dots).
...
% slide Oberser-Observable pattern

\subsubsection{Introduzione ai 3 componenti principali}
% Slide di presentazione dei 3 componenti principali
\par \textbf{Activity/Fragment} è il Lifecycle Owner. Ovvero implementa o estende l'observer (i cui metodi sono callback dell'activity e che sono observer di tutti gli stati della LifeCycle che il LifecycleOwner ha associato).
...
% Slide con implementazione, "TestObserver" è il mio "MyObserver" della slide "Funzionamento".
% CI sono due interfacce, Java 7 è più vecchia e deprecata, l'altra è più recente e sarà quella che andremo ad usare noi a partire da Java 8.

\subsubsection{Funzionamento}
% Slide 
MyObserver è la classe che IO implemento (perché IO voglio gestire cosa succede quando cambia di stato) e che estende quella Default di Android che è Observer dell'Activity.
...
\par Notifica gli observer quando i dati cambiano di valore ("aggiornamento dal basso").
...
\end{itemize}
% Slide con schema e frasetta utile
...
% AnObserver vuole registrarsi come observer ed essere notificato ("this") dei cambiamenti di stato del LifecycleOwner (activity/fragment).
% spiega bene il flusso "evento che mi cambia generando una azione in risposta"
...
% Fa, se due fragment di un'activity comunicano fra loro, si appoggiano ai loro ViewModel (ciascuno ne ha uno) e comunicano fra loro attraverso il ViewModel dell'Activity. Es: un VM per le News un VM per glu users, ciascuno è un singleton.
...
% LiveData observer Activity, notifica se active.
...
% Le ultime 25 slide direi guardale le ha passate veloci veloci